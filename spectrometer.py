...
Occasionaly the program throws a div0 warning because of the peak detector. You can sefely ignore it, it is just a warning!
...
class App:
	def __init__(self, window, window_title, video_source=0):
		...
		def snapshot():
			...
			ret, img = self.vid.get_graph()
			if ret:
				cv2.imwrite("spectrum-" + time.strftime("%d-%m-%Y-%H:%M:%S") + ".jpg", cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
		...
		#peaks label
		self.lbpeak = tkinter.Label(self.bottom_frame  ,text = "Label Peak Width:")
		self.lbpeak.grid(row = 1,column = 0, pady=20)

		#slider for peak width
		self.peakwidth = tkinter.Scale(self.bottom_frame,from_=0, to=100, orient="horizontal", command=peakwidth)
		self.peakwidth.grid(row=1, column=1, padx=0, pady=0, sticky="n")
		self.peakwidth.set(50)	

		#threshold label
		self.lbthresh = tkinter.Label(self.bottom_frame  ,text = "Label Threshold:")
		self.lbthresh.grid(row = 1,column = 2, pady=20)

		#slider for threshold
		self.thresh = tkinter.Scale(self.bottom_frame, from_=0, to=100, orient="horizontal", command=peakthresh)
		self.thresh.grid(row=1, column=3, padx=0, pady=0, sticky="n")
		self.thresh.set(20)	
	
		#Snapshot the graph
		self.snapshotbtn = tkinter.Button(self.bottom_frame, text="Snapshot", width=15, command=snapshot)
		self.snapshotbtn.grid(row=1, column=4, padx=0, pady=0)		
	...
	def update(self):
		...
		ret2, graph = self.vid.get_graph()
		if ret:
			self.photo = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(frame))
			self.canvas0.create_image(0, 0, image = self.photo, anchor = tkinter.NW)
		if ret2:
			self.photo2 = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(graph))
	...
	def get_graph(self):
		...
		if self.vid.isOpened():
			...
			if ret:
				...
				intensity = []
				#pull out  single row of data and store in a 1D array
				...
				intensity = savgol_filter(intensity,17,7)
				...
				indexes = peakutils.indexes(intensity, thres=thresh/max(intensity), min_dist=self.mindist)
				for i in indexes:
					height = intensity[i]
					height = 245-height
